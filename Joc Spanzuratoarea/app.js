...
function alegeCuvant(){
    let cuvinte = ['ciocan','autobuz','nicovala', 'portocala', 'trandafir'];
...
function deseneazaCuvant(cuvant){
    ...
    cuvant.split('').forEach((l,i)=>{
       let litera = document.createElement('span');
...
function genereazaNumar(minValue, maxValue){
    return Math.ceil (minValue + Math.random() *
    (maxValue-minValue))
}
...
function onKeyUp(e){
   ...
   if(getEmptySlots().length==0){
    ...
    alert('Ai pierdut ! Te spanzuram :D');
